...
def get_movies_from_tastedive(movieName):
    param = {'q': movieName , 'limit': 5, 'type': 'movies'}
    resp = requests_with_caching.get('https://tastedive.com/api/similar', params=param)
...
def extract_movie_titles(Dict):
    return ([i['Name'] for i in Dict['Similar']['Results']])
...
def get_related_titles(lst):
    List = []
    for i in lst:
        List.extend(extract_movie_titles(get_movies_from_tastedive(i)))
...
def get_movie_data(movieName):
    param = {'t': movieName , 'r': 'json'}
    resp = requests_with_caching.get('http://www.omdbapi.com/', params=param)
    return json.loads(resp.text)
